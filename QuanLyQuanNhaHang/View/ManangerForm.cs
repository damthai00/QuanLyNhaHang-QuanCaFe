...
namespace QuanLyQuanCafe
{
    public partial class ManangerForm : Form
    {
        ...
        {
            QuanLyNhaHangForm f = new QuanLyNhaHangForm(this);
            f.ShowDialog();
        }
        ...
        {
            DataTable dtban = XuLyBanController.Instance.getBanSuDung2();
            ...
            //List<ThucDonHienThi> dt = XuLyThucDonController.Instance.XuLyThucDonConHang();
            DataTable dtThucDon = XuLyThucDonController.Instance.getThucDonConHang();
        ...
        public void loadConlai()
        {
            DataTable dt = XuLyThucDonController.Instance.getThucDonTheoID(Convert.ToInt32( cbbMaTDChon.Text));
            if(dt!=null)
                foreach (DataRow item in dt.Rows)
                {
                    txtSoLuongTDConLai.Text = item["soluong"].ToString();
                }
            nbrSoLuong.Value = 0;
        }
        ...
        private void dtgThucDon_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if(pn_goimon.Enabled == true && dong >=0)
            {
                ...
                txtSoLuongTDConLai.Text = dtgThucDon.Rows[dong].Cells["sl"].Value.ToString();
                txtDonGiaTDChon.Text = dtgThucDon.Rows[dong].Cells["dg"].Value.ToString();
            }
        }

        public int idBanClick;
        private void button5_Click(object sender, EventArgs e)
        {
            int id_ban = this.idBanClick;
            if(XuLyBanController.Instance.checkBan(id_ban))
            {
                TinhTienForm f = new TinhTienForm(this);
                f.NhanDuLieu(id_ban);
                f.ShowDialog(this);
            }
        ...
        private void dtgCTHD_CellClick(object sender, DataGridViewCellEventArgs e)
        {
             int dong = e.RowIndex;
            
            if(dong >=0)
            {
                String tenTD =dtgCTHD.Rows[dong].Cells["ten_thucdon"].Value.ToString();
                int id   =Convert.ToInt32( dtgCTHD.Rows[dong].Cells["ma_cthd"].Value);
                int id_ban = XuLyCTHoaDonController.Instance.getIDBanByIDCTHD(id);
                float thanhtien = Convert.ToInt32(dtgCTHD.Rows[dong].Cells["thanhtien"].Value);
                DialogResult dr = MessageBox.Show("Bạn muốn xóa: "+tenTD,"Thông báo",MessageBoxButtons.YesNo,MessageBoxIcon.Warning);
                ...
                {              
                    ...
                    {
                        if (XuLyCTHoaDonController.Instance.XuLyXoaCTThucDon(id,thanhtien))
                        {
                            this.loadBan();
                            this.loadCBBBan();
                            this.loadDSThucDon();
